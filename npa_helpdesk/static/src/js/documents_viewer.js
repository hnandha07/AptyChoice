odoo.define('npa_helpdesk.DocumentViewer', function (require) {
"use strict";
...
var DocumentViewer = Widget.extend({
    ...
    events: {
        ...
        'click .o_checked_btn': '_onChecked',
        'click .o_reject_btn': '_onReject',
        'click .o_archive_btn': '_onArchive',
    ...
    init: function (parent, attachments, activeAttachmentID) {
        ...
        this.attachment = _.filter(attachments, function (attachment) {
            var match = attachment.file_name.match("(.pdf|.txt|.png|.jpg|.jpeg|.gif|.PNG|.JPEG|.JPG|.GIF)");
            if (match) {

                attachment.type = match[1];
                if (match[1].match("(.png|.jpg|.gif|.jpeg|.PNG|.JPEG|.JPG|.GIF)")) {
                    attachment.type = 'image';
                } else if (match[1].match("(.pdf|.txt|.xlsx)")) {
                    attachment.type = 'application/pdf';
                } else if (match[1] === 'youtu') {
    ...
    _onChecked: function (e) {
        ...
        if (this.activeAttachment){
            this._rpc({
                model: 'npa.document',
                method: 'checked_document',
                args: [this.activeAttachment.id],
            });
            this._reset();
            self.__parentedParent._setValue({ operation: 'UPDATE', id: self.__parentedParent.record.id });
        }
    },
    ...
    _onDownload: function (e) {
        e.preventDefault();
        window.location = '/web/binary/download_document/' + this.activeAttachment.id ;
    },
    /**
     * @private
     * @param {MouseEvent} e
